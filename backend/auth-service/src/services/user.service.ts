...
export const findUserByEmail = (email: string): SafeAuthUser | null => {
    ...
    const stmt = db.prepare(`
        SELECT id, name, email, twoFactorEnabled, createdAt, updatedAt
        FROM ${TABLE_NAME}
        WHERE email = ?
    `);
...
export const findUserByEmailWithSensitiveData = (email: string): AuthUser | null => {
    const db = getDb();
    const stmt = db.prepare(`
        SELECT id, name, email, password, twoFactorEnabled, twoFactorSecret, createdAt, updatedAt
        FROM ${TABLE_NAME}
        WHERE email = ?
    `);
    ...
    if (!user)
        return null;
    // Convert twoFactorEnabled back to boolean
    return {
        ...user,
        twoFactorEnabled: sqliteToBool(user.twoFactorEnabled as any),
        createdAt: new Date(user.createdAt * 1000).toISOString() as any,
...
export const createUser = async (userData: NewAuthUser): Promise<SafeAuthUser | null> => {
    ...
    const stmt = db.prepare(`
        INSERT INTO ${TABLE_NAME} (name, email, password, twoFactorEnabled, twoFactorSecret, createdAt, updatedAt)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    try {
        const info = stmt.run(
 ...
 */
export const updateUserTwoFactor = async (userId: number, secret: string | null, enabled: boolean): Promise<boolean> => {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    const stmt = db.prepare(`
        UPDATE ${TABLE_NAME}
